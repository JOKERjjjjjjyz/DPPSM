...
class DP_BackoffModel(ngram_chain.NGramModel):
    ...
    def __init__(self, words, threshold, start_symbol=True,
        ...
        state_histogram = [charcounts[char] for char in charcounts]
        epsilon_remain = epsilon_total / 2
        epsilon = epsilon_remain / 9
        # new_histogram_counts = DP_util.apply_exponential_mechanism(state_histogram, epsilon_remain, delta_u=delta_u1)
        # for i, char in enumerate(charcounts):
        #     charcounts[char] = new_histogram_counts[i]
        for i, char in enumerate(charcounts):
                charcounts[char] = max(1, charcounts[char] + numpy.random.laplace(0, 2 * (U-lendelta) / epsilon_remain, size=1).item())
        ...
        for n in range(2, 11):
            ...
            state_histogram = [ngram_counter[ngram] for ngram in ngram_counter]
            new_histogram_counts = DP_util.apply_exponential_mechanism(state_histogram, epsilon/2, delta_u=2 * (U + 2 - n))
            for i, ngram in enumerate(ngram_counter):
                noise_ngram_counter[ngram] = new_histogram_counts[i]
            for ngram, count in noise_ngram_counter.items():
                noise_dict[ngram[:-1]].append((ngram[-1], count))

            for (state, sscounts), (state2, sscounts_with_noise) in zip(tmp_dict.items(), noise_dict.items()):
                if state!=state2:
                    print("false!")
                total = sum(count for _, count in sscounts)
                if total < threshold:
                    continue
                total_noise = sum(count for _, count in sscounts_with_noise)
                trans_probs = {c: count_noise / total_noise
                            #    for c, count in sscounts
                               for (c, count), (c2, count_noise) in zip(sscounts, sscounts_with_noise)
                               if DP_util.AboveThreshold(count, threshold_DP, epsilon=(epsilon/2), delta_u = 2*(U+2-n)) and c == c2}
