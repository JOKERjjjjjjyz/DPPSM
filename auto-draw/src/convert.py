...
This is a method to convert monte carlo result to json file, which easy to draw figure
"""
import argparse
import json
import sys
...
def jsonify(label: str, fd_gc: TextIO, fd_save: TextIO, 
    ...
    lst = []
    for line in fd_gc:
        hits, guesses = key(line)
        if guesses < lower_bound:
            continue
        if guesses > upper_bound:
            break
        guesses_list.append(int(guesses))
        cracked_list.append(int(hits))
    fd_gc.close()

    if(len(guesses_list) == 1):
        sys.stderr.write(f'empty password list: {fd_gc.name}\n')
...
def main():
    ...
    jsonify(label=args.label, fd_gc=args.fd_gc, fd_save=args.fd_save, 
            fd_test=args.fd_test, 
