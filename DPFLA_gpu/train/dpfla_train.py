...
class DPTrainer:
    def __init__(self, config_path, data_path):
        ...
        self.model_save_path = './models/model/rockyou_new_dpfla_0.1293.pth'
    ...
    def train(self):
        for epoch in range(self.epoch_size):
            ...
            if (epoch+1) % 5 == 0:
                os.makedirs(os.path.dirname(f"./models/model/rockyou_new_dpfla_0.1293_{epoch}.pth"), exist_ok=True)
                torch.save(self.model.state_dict(), f"./models/model/rockyou_new_dpfla_0.1293_{epoch}.pth")
                print(f"Model saved to ./models/model/rockyou_new_dpfla_0.1293_{epoch}.pth")
            if epoch == 0:
                os.makedirs(os.path.dirname(f"./models/model/rockyou_new_dpfla_0.1293_{epoch}.pth"), exist_ok=True)
                torch.save(self.model.state_dict(), f"./models/model/rockyou_new_dpfla_0.1293_{epoch}.pth")
                print(f"Model saved to ./models/model/rockyou_new_dpfla_0.1293_{epoch}.pth")
        ...
        with open("./models/model/losses_rock_city_dpfla_0.1293.json", "w") as f:
            json.dump(self.smoothed_losses, f)
        print("Smoothed losses saved to ./models/model/losses_rock_city_dpfla_0.1293.json")
