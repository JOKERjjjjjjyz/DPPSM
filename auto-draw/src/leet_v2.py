...
class AsciiL33tDetector:
    ...
    def find_l33t(self, word: str) -> Tuple[bool, str]:
        """
...
def wrapper():
    ...
    cli.add_argument("-m", "--model", dest="model", type=argparse.FileType('wb'), default=None)
    cli.add_argument("-l","--load", dest="load", type=argparse.FileType('rb'), default=None)
    args = cli.parse_args()
    f_out = args.output  # type: TextIO
    if not f_out.writable():
        print(f"{f_out.name} is not writable", file=sys.stderr)
        sys.exit(-1)
    if args.load != None and args.model == None:
        leet_detector = pickle.load(args.load)
    else:
        leet_detector = obtain_leet_detector(corpus=args.corpus)
        if args.model != None:
            pickle.dump(leet_detector, args.model)
            args.model.close()
    ...
    for pwd, leets in pwd_leet_dict.items():
        cnt = pwd_dict[pwd]
        f_out.write(f"{pwd}\t{cnt}\t{leets}\n")
