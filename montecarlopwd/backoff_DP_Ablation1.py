...
class DP_BackoffModel(ngram_chain.NGramModel):
    ...
    def __init__(self, words, threshold, start_symbol=True,
                 with_counts=False, epsilon_total=1.0, shelfname=None):
        ...
        epsilon = epsilon_total / lens[-threshold]
        new_histogram_counts = DP_util.apply_exponential_mechanism(state_histogram, epsilon, delta_u=delta_u1)
        ...
        for n in range(2, lens[-threshold] + 1):
            ...
            threshold_DP = threshold + numpy.random.laplace(0, 2*2*(U+1-n) / epsilon, size=1).item()
            ...
            for state, sscounts in tmp_dict.items():
                ...
                trans_probs = {c: sscounts_with_noise[c] / total_noise
                               ...
                               if DP_util.AboveThreshold(count, threshold_DP, epsilon=epsilon, delta_u = 2*(U+1-n))}
