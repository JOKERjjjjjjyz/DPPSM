...
if [ "$data" == "cityday" ]; then
    DATA=(
        "/home/guest/zsf/jyz/montecarlopwd/result/rock-city-DPbackoff10gram-0.1.csv" \
        "/home/guest/zsf/jyz/montecarlopwd/result/rock-city-DPbackoff10gram-1.0.csv" \
        "/home/guest/zsf/jyz/montecarlopwd/result/rock-city-DPbackoff10gram-2.csv" \
    ...
    DATA=(
        "/home/guest/zsf/jyz/montecarlopwd/result/rock2019-rock2024-DPbackoff10gram-0.1.csv" \
        "/home/guest/zsf/jyz/montecarlopwd/result/rock2019-rock2024-DPbackoff10gram-1.0.csv" \
        "/home/guest/zsf/jyz/montecarlopwd/result/rock2019-rock2024-DPbackoff10gram-2.csv" \
...
do
    ...
    # JSONS+="${BASE}/json/${json_filename} "
    JSONS+="${BASE}/json_no_marker/epsilon/${json_filename} "
    # python3 ${BASE}/src/convert_v2.py \
    python3 ${BASE}/src/convert_dppsm.py \
    -l "$label" \
    -f $dataset \
    -s "${BASE}/json_no_marker/epsilon/${json_filename}" \
    -t ${TEST} \
...
--suffix .pdf                               \
-s ${BASE}/figure/DPPSM/$data"_dpbackoff_epsilon_result.pdf"               \
-x 'Guesses'                               \
-y 'Cracked (%)'                            \
--show-text                                 \
--legend-loc 'none'                   \
