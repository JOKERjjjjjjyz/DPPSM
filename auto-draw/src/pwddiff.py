...
from typing import TextIO, Tuple, Callable, Iterable
import re
...
def diff(finput:TextIO, target:TextIO, output:TextIO, 
        ...
        sort:str, max_guess:int, pwd_only:bool):
...
def main():
    ...
    cli.add_argument("--input-freq-index",required=False, dest="input_freq",type=int, default=-1, 
                    help="Frequency index in input file")
    ...
    def my_key(line: str):
        try:
            ...
            freq = 1
            if args.input_freq != -1:
                freq = int(split_line[args.input_freq])
            return split_line[args.input_pwd], freq, guess
            ...
            print(f"Your split is '{split}',\n"
                  ...
                  f"    input_freq is '{args.input_freq}'", file=sys.stderr)
            sys.exit(-1)
    ...
    diff(args.input, args.target, args.output, 
        ...
        args.sort_by, args.max_guess, args.pwd_only)
