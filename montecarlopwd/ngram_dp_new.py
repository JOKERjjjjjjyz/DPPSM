...
class NGramModel(model.Model):
    ...
    def __init__(self, words, n, epsilon, with_counts=False, shelfname=None):
        ...
        ngrams_counter_instance = ngrams_counter(words, n, start, end, with_counts)
        state_histogram = [count for count in ngrams_counter_instance.values()]
        new_histogram_counts = DP_util.apply_exponential_mechanism(state_histogram, epsilon, delta_u=noise_scale)
        for i, ngram in enumerate(ngrams_counter_instance):
            ngrams_counter_instance[ngram] = new_histogram_counts[i]

        for ngram, count in ngrams_counter_instance.items():
