...
class Trainer:
    def __init__(self, config_path, data_path):
        ...
        self.password_dataset = PasswordDataset(txt_path=data_path)
        self.preprocessor = Preprocessor(config_path=config_path)
        self.model = LSTMModel(config_path=config_path).to(self.device)  # 模型移动到 GPU
        self.criterion = nn.CrossEntropyLoss(reduction='none').to(self.device)  # 损失函数也在 GPU 上
        ...
        self.smoothed_loss = 0
        self.alpha = 0.9  # 平滑系数
        self.model_save_path = f"{self.config.get('model_file', './models/model/rockyou_new.pth')}"
    ...
    def train_step(self, pwd_list):
        ...
        # 将数据移动到 GPU 上
        x_batch = torch.cat(x_batches, dim=0).to(self.device)
        y_batch = torch.cat(y_batches, dim=0).to(self.device)
        w_batch = torch.cat(w_batches, dim=0).to(self.device)

        # 进行一个 batch 的训练步骤
        self.optimizer.zero_grad()
        outputs = self.model(x_batch)  # x_batch 的形状为 (batch_size, seq_len)
        loss = self.criterion(outputs, y_batch)  # y_batch 是字符索引
        weighted_loss = (loss * w_batch).mean()
        weighted_loss.backward()
        self.optimizer.step()
