...
class DPSGDLSTMModel(nn.Module):
    def __init__(self, config_path):
        super(DPSGDLSTMModel, self).__init__()
        with open(config_path, 'r') as config_file:
            self.config = json.load(config_file)

        input_size = self.config.get('vocab_size', 97)  # 默认97个字符
        ...
        self.lstm = DPLSTM(lstm_input_size, hidden_size, num_layers, batch_first=True)
    ...
    def forward(self, x):
        ...
        out, _ = self.lstm(x)  # LSTM 输入为 (batch_size, seq_len, input_size)
