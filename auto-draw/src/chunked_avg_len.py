...
def read_pwd_file(file:TextIO):
    ...
    for line in file:
        ...
        num = len(line.split(' '))
        #print(num)   
        pwd_chunks_num.append(num)
    return pwd_chunks_num
...
def create_args():
    ...
    cli.add_argument("-i", dest="input", type=argparse.FileType("r"), required=True)
...
def main():
    ...
    pwd_chunks_nums = read_pwd_file(args.input)
    total_chunk=0
    total_chunk=np.sum(pwd_chunks_nums)
    length=len(pwd_chunks_nums)
    Avg_chunk_num=float(total_chunk/length)
    Avg_chunk_num=round(Avg_chunk_num,3)
    print(f"Avg_chunk_num:{Avg_chunk_num}")
