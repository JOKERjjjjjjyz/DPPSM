...
labels = [
    "CKL_Backoff",
    "$\texttt{Neopets}$  CKL_PCFG",
    "CKL_FLA",
    "CKL_Backoff",
    "$\texttt{Cit0day}$   CKL_PCFG",
    "CKL_FLA",
    "CKL_Backoff",
    "$\texttt{Youku}$     CKL_PCFG",
    "CKL_FLA",
    "CKL_Backoff",
    "$\texttt{178}$         CKL_PCFG",
    "CKL_FLA"
...
def plot_bar(items: List[CellItem]):
    handles = []
    plt.rcParams['hatch.linewidth'] = 15.0
    ...
    lims = [210000, 980000, 210000, 1100000]
    ...
    for i, each_subplot in enumerate(four_subplots):
        ...
        #axes[i].set_xlabel(f'{legend[i]}', fontsize=8)
...
def main():
    ...
    plt.savefig("/disk/xm/auto-draw/figure/Bar/pattern-comparison.pdf", bbox_inches='tight')
    plt.close(figure)
...
def draw_legend():
    ...
    plt.barh([2], [0], color='red', label="Number (CKL_model)")
    legend = plt.legend(loc='center', fontsize=28, handlelength=2.0, handletextpad=0.1, frameon=False, labelspacing=0.2,
                        ncol=2)
