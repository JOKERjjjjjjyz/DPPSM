...
from utils.preprocessing import SubstringDictGenerator, DPSubstringDictGenerator, Preprocessor_Ablation1
...
class DPTrainer:
    def __init__(self, config_path, data_path):
        ...
        self.preprocessor = Preprocessor_Ablation1(config_path, self.DPsubstring_dict, self.pwd_max_len)
        ...
        self.model_save_path = './models/ablation/rockyou_new_abla1_1.2149.pth'
        ...
        self.noise_multiplier_account = 1.5
    ...
    def train(self):
        for epoch in range(self.epoch_size):
            ...
            if (epoch+1) % 5 == 0:
                os.makedirs(os.path.dirname(f"./models/ablation/rockyou_new_abla1_0.1293_{epoch+1}.pth"), exist_ok=True)
                torch.save(self.model.state_dict(), f"./models/ablation/rockyou_new_abla1_0.1293_{epoch+1}.pth")
                print(f"Model saved to ./models/ablation/rockyou_new_abla1_0.1293_{epoch+1}.pth")
            if epoch == 0:
                os.makedirs(os.path.dirname(f"./models/ablation/rockyou_new_abla1_0.1293_{epoch+1}.pth"), exist_ok=True)
                torch.save(self.model.state_dict(), f"./models/ablation/rockyou_new_abla1_0.1293_{epoch+1}.pth")
                print(f"Model saved to ./models/ablation/rockyou_new_abla1_0.1293_{epoch+1}.pth")
        ...
        with open("./models/ablation/losses_rock_city_abla1_0.1293.json", "w") as f:
            json.dump(self.smoothed_losses, f)
        print("Smoothed losses saved to ./models/ablation/losses_rock_city_abla1_0.1293.json")
