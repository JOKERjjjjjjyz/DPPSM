...
# epsilon_values = [1.0, 0.2]
epsilon_values = [2.0]
for epsilon in epsilon_values:
    models = {}
    start = time.time()
    shelf_path = f"./model/4gram_rockyou_2019/dp4gram_new_model_rockyou2019_{epsilon}.db"
    # models['4gram']=ngram_dp_new.NGramModel.get_from_shelf(shelfname=shelf_path, n=4, epsilon = epsilon)
    models['4gram'] = ngram_dp_new.NGramModel(words=train_data, n=4,epsilon=epsilon,shelfname= shelf_path)
    ...
    # 将结果保存成csv文件
    output_file_path = f"./result/rock2019-rock2024-4gram-dp-new-{epsilon}.csv"  # 设置你的输出文件路径
