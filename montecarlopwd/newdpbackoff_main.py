...
# 对于不同的 epsilon 进行实验
# epsilon_values = [0.2, 0.01]
# epsilon_values = [0.1, 1.0, 2, 5, 10]
epsilon_values = [1.0]

for epsilon in epsilon_values:
    models = {}
    shelf_path = f"./model/backoff_rockyou_2019/DP_newbackoff_10gram_model_rockyou2019_{epsilon}.db"
    # models[str(epsilon)] = newdpbackoff.DP_BackoffModel.get_from_shelf(shelf_path, threshold=10)
    models[str(epsilon)] = newdpbackoff.DP_BackoffModel(train_data, threshold=10, epsilon_total=epsilon,shelfname=shelf_path)
    ...
    # 将结果保存成csv文件
    output_file_path = f'./result/rock2019-rock2024-newDPbackoff10gram-{epsilon}.csv'  # 设置你的输出文件路径
