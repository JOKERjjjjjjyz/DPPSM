...
models = [
    # neopets
    [23969,3303472,972461  ,3858176 ],
    [23969,2832232,971822 , 2370278 ],
    [23969,3291593,971125 , 2344374 ],
    # cit0day
    [5945,4893697, 1066036, 3858176 ],
    [5945,5072655, 1067389, 4035776 ],
    [5945,5138174,1069230 , 4099458 ],
    # youku
    [4417,4184321, 547061, 3643166 ],
    [4417,4251214, 546458, 3709928 ],
    [4417,3568508,534001 , 3040416 ],
    # 178
    [3108,623945,184007 , 443046 ],
    [3108,629257,183951 , 448414 ],
    [3108,578187,182918 , 398377 ]
]

base = [
    # neopets
    [23925, 3119922,941805 , 398377 ],
    [17275, 3318192,827985 , 2021464 ],
    [23969,2188543,799138 , 1413318 ],
    # cit0day
    [5929, 4407439, 1023417, 3414529 ],
    [2413, 4550423, 943040, 3634608 ],
    [5945, 3549848,839570 , 2739414 ],
    # youku
    [4381, 4013278, 526137, 3492161 ],
    [4359,1831051, 262121, 1574033 ],
    [4415, 1545840,372570 , 1177685 ],
    # 178
    [3101,605461,182658 , 425904 ],
    [3104,320416, 118855, 204665 ],
    [3107,320850,148538 , 175419 ]
]
...
def plot_bar(items: List[CellItem]):
    handles = []
    plt.rcParams['hatch.linewidth'] = 100.0
    ...
    sub_left.text(0.5, -0.0175, "(FLA)", ha="center", fontsize=8)
    sub_right.text(0.0001, -0.0175, "Number", ha="center", fontsize=8)
    sub_right.text(0.5, -0.0175, "(zxcvbn)", ha="center", fontsize=8)
    axes = (ax1, ax2, ax3, ax4)
    # print(ax[0])
    #ends = [3000000, 1000000, 2000000, 7500]
    ends = [20000,4200000,800000,3000000]
    lims = [26000, 5290000, 1110000, 4200000]
    # lims = [5500000, 2500000, 4200000, 12000]
    #lims = [5600000, 2600000, 4303000, 13000]
    end_labels = [r'$2\times10^{4}$', r'$4.2\times10^{6}$', r'$8\times10^{5}$', r'$3\times10^{6}$']

    # legend = ["Medium", "Strong", "Medium", "Strong", "\n", "FLA", " " ,"zxcvbn"]
    # legend = ["Medium", " (FLA)", "Strong", "Medium", "(zxcvbn)", "Strong"]
    legend = ["Weak", "Medium&Strong", "Weak", "Medium&Strong"]
    ...
    for i, each_subplot in enumerate(four_subplots):
        ...
        axes[i].tick_params(axis='x', which='major', direction="in", labelsize=8)
        axes[i].tick_params(axis='y', which='major', length=0, labelsize=8)
        ...
        drew_legend = axes[i].legend([wanted_legend], [legend[i]], loc='lower right', fontsize=8, handlelength=0.0,
                                     handletextpad=0.0, bbox_to_anchor=(1.06, 0.01),
                                     frameon=False)
    ...
    index2 = index[1::2]
    index = index[0::2]
    masks = [-20, 0, 30] * (config.types // 3)
    masks2 = [10, 30, 60] * (config.types // 3)
    index += masks
    index2 += masks2
    adjust = [-2, -2, -2] * (config.types // 3)
    axes[0].set_yticks(index + adjust)
...
def main():
    ...
    plt.savefig("/disk/xm/auto-draw/figure/Bar/strength-comparison.pdf", bbox_inches='tight')
...
def draw_legend():
    ...
    legend = plt.legend(loc='center', fontsize=39, handlelength=2.0, handletextpad=0.1, frameon=False, labelspacing=0.1,
                        ncol=2)
