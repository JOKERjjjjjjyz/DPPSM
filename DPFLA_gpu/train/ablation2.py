...
from utils.preprocessing import Preprocessor_Ablation2
...
class DPTrainer:
    def __init__(self, config_path, data_path):
        ...
        self.preprocessor = Preprocessor_Ablation2(config_path, self.pwd_max_len)
        ...
        self.model_save_path = './models/model/rockyou_new_dpfla_0.01.pth'
        ...
        self.noise_multiplier_account = 8
        self.noise_multiplier = 2 * self.noise_multiplier_account
    ...
    def train_epoch(self,epoch):
        for batch_idx in range(self.batch_per_epoch):
            ...
            if (batch_idx + 1) % self.batch_print_size == 0:
                print(f"Epoch {epoch+1}, Batch [{batch_idx + 1}/{self.batch_per_epoch}], Smoothed Loss: {self.smoothed_loss:.4f}", flush=True)
    ...
    def train(self):
        for epoch in range(self.epoch_size):
            ...
            if (epoch+1) % 5 == 0:
                os.makedirs(os.path.dirname(f"./models/model/rockyou_new_dpfla_0.01_{epoch}.pth"), exist_ok=True)
                torch.save(self.model.state_dict(), f"./models/model/rockyou_new_dpfla_0.01_{epoch}.pth")
                print(f"Model saved to ./models/model/rockyou_new_dpfla_0.01_{epoch}.pth")
            if epoch == 0:
                os.makedirs(os.path.dirname(f"./models/model/rockyou_new_dpfla_0.01_{epoch}.pth"), exist_ok=True)
                torch.save(self.model.state_dict(), f"./models/model/rockyou_new_dpfla_0.01_{epoch}.pth")
                print(f"Model saved to ./models/model/rockyou_new_dpfla_0.01_{epoch}.pth")
        ...
        with open("./models/model/losses_rock_city_dpfla_0.01.json", "w") as f:
            json.dump(self.smoothed_losses, f)
        print("Smoothed losses saved to ./models/model/losses_rock_city_dpfla_0.01.json")
