...
class DP_BackoffModel(ngram_chain.NGramModel):
    ...
    def __init__(self, words, threshold, start_symbol=True,
        ...
        self.ngram_flag_table = load_ngram_flag_table('ngram_flag_table.json')
        if not with_counts:
        ...
        # delta = delta_total / (lens[-threshold]-1)
        ...
        for n in range(2, lens[-threshold] + 1):
            ...
            # 创建一个新的字典，只保留出现在 ngram_flag_table 中的 n-gram
            filtered_ngram_counter = {}
            
            for ngram, count in ngram_counter.items():
                if ngram in self.ngram_flag_table:
                    filtered_ngram_counter[ngram] = count  # 保留
                else:
                    discarded_count += 1  # 统计被丢弃的条目
            remaining_count = len(filtered_ngram_counter) 

            print(f"被丢弃的 n-gram 对数: {discarded_count}")
            print(f"剩余的 n-gram 对数: {remaining_count}")

            tmp_dict = collections.defaultdict(list)
            for ngram, count in filtered_ngram_counter.items():
